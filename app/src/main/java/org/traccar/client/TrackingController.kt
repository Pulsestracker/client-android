...
import org.traccar.client.RequestManager.sendUdpData
...
class TrackingController(private val context: Context) : PositionListener, NetworkHandler {
    ...
    private val buffer: Boolean = false
    ...
    private fun createPayload(position: Position): String {
        ...
        // Basic app and device information
        json.put("clientId", position.deviceId)
        // Location data
        val data = JSONObject()
        data.put("type", "Point")
        val coordinates = JSONArray()
        coordinates.put(position.longitude)
        coordinates.put(position.latitude)
        data.put("coordinates", coordinates)
        json.put("data", data)

        // Additional information
        val extra = JSONObject()
        extra.put("battery", position.battery)
        extra.put("charging", position.charging)
        extra.put("time", position.time)
        extra.put("altitude", position.altitude)
        extra.put("accuracy", position.accuracy)
